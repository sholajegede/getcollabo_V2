...
export interface ForgotPasswordBrandProps {
  className?: string;
}

const ForgotPasswordBrand: FC<ForgotPasswordBrandProps> = ({ className = "" }) => {
  ...
  const handleReset = async (e) => {
    e.preventDefault();
    try {
      const res = await newRequest.post("/auth-brand/forgot-password", userEmail);
      console.log(res.status);
      history.push(`/verifyBrandOTP/${userEmail.email}`);
    } catch (error) {
  ...
  return (
    <div
      className={`nc-ForgotPasswordBrand ${className}`}
      data-nc-id="ForgotPasswordBrand"
    >
      <Helmet>
...
export default ForgotPasswordBrand;
