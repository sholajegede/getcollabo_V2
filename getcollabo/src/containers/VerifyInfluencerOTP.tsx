...
const VerifyInfluencerOTP: FC<VerifyInfluencerOTPProps> = ({ className = "" }) => {
  ...
  const handleVerification = async (e) => {
    ...
    try {
      ...
      const verify = await newRequest.post(`/auth-influencer/verify-password-reset/${influencer._id}`, newOTP);
      console.log(verify.status);
      history.push(`/reset-password-creator/${influencer.email}`);
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className={`nc-VerifyInfluencerOTP ${className}`} data-nc-id="VerifyInfluencerOTP">
      <Helmet>
        <title>Verify Creator OTP | GetCollabo</title>
      </Helmet>
