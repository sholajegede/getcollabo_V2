...
const InfluencerLogin: FC<InfluencerLoginProps> = ({ className = "" }) => {
  ...
  return (
    <div className={`nc-PageLogin ${className}`} data-nc-id="PageLogin">
      <Helmet>
        <title>Creator Login - GetCollabo</title>
      </Helmet>
      <div className="container mb-24 lg:mb-32">
        ...
        <div className="max-w-md mx-auto space-y-6">
            ...
            <form className="grid grid-cols-1 gap-6">
              ...
              <label className="block">
                <span className="flex items-center justify-between text-neutral-800 dark:text-neutral-200">
                  Password
                  <Link to="/forgot-password-creator" className="text-sm text-green-600">
                    Forgot password?
                  </Link>
                </span>
              ...
              <ButtonPrimary disabled={loading} type="button" onClick={handleLogin}>Log In</ButtonPrimary>
              <ToastContainer className="text-sm" />
              {error && <p className="justify-center text-sm text-center text-red-500">{error}</p>}
            </form>
          </div>
          ...
          <span className="block text-center text-neutral-700 dark:text-neutral-300">
            First time? {` `}
            <Link className="text-green-600" to="/create-profile">
              Create a profile
            </Link>
          </span>
        </div>
